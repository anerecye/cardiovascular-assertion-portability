...
# Arrhythmia assertion-level sources accepted for three-domain decision challenge.
# The first two are preferred because they contain assertion-level future drift endpoints.
ARR_ASSERTION_SOURCE_CANDIDATES = [
    DATA / "cab_predictive_operational_framework.csv",
    DATA / "cab_three_domain_arrhythmia_decision_source.csv",
    DATA / "cab_arrhythmia_decision_source.csv",
    DATA / "cab_gene_architecture_temporal_alignment.csv",
    DATA / "cab_cross_environment_drift.csv",
]
...
def first_nonempty_source(paths: List[Path]) -> tuple[pd.DataFrame, str]:
    for path in paths:
        df = safe_read(path)
        if not df.empty:
            return df, str(path.relative_to(BASE))
    return pd.DataFrame(), ""


def coalesce_existing(df: pd.DataFrame, target: str, candidates: List[str], default=np.nan):
...
def merge_arrhythmia_sources(base: pd.DataFrame, reg: pd.DataFrame) -> pd.DataFrame:
    """Merge assertion-level arrhythmia endpoints with baseline-only CPI/regime features."""
    df = base.copy()
    if not reg.empty:
        # Prefer assertion_id join; fall back to variation_id. Avoid suffixing endpoint columns.
        keep = [c for c in reg.columns if c in {"assertion_id", "variation_id"} or c.startswith("CPI_") or c.startswith("baseline_") or c in {
            "nonportability_score_baseline_only", "CPI_tier_baseline_only",
            "included_features", "excluded_features_due_to_leakage"
        }]
        reg2 = reg[keep].copy()
        if "assertion_id" in df.columns and "assertion_id" in reg2.columns:
            df = df.merge(reg2, on="assertion_id", how="left", suffixes=("", "_cpi"))
        elif "variation_id" in df.columns and "variation_id" in reg2.columns:
            df = df.merge(reg2, on="variation_id", how="left", suffixes=("", "_cpi"))
    return df
...
def fdr_bh(pvals):
    """Benjamini-Hochberg FDR correction with NaN treated as 1.0."""
    ...
    for rank_rev, idx in enumerate(order[::-1], start=1):
        rank = m - rank_rev + 1
        val = min(min_adj, p[idx] * m / max(rank, 1))
        min_adj = val
        adj[idx] = min(val, 1.0)
    return adj.tolist()
...
def load_domain_records(domain: str) -> pd.DataFrame:
    ...
    if domain == "inherited_arrhythmia":
        base, source_name = first_nonempty_source(ARR_ASSERTION_SOURCE_CANDIDATES)
        reg = safe_read(ARR_REGIME)
        if base.empty:
            raise FileNotFoundError(
                "Three-domain decision challenge requires an assertion-level inherited-arrhythmia source. "
                "Copy data/processed/cab_predictive_operational_framework.csv from the working CAB project into this repo, "
                "or provide data/processed/cab_three_domain_arrhythmia_decision_source.csv with assertion_id/gene/baseline portability/endpoints."
            )
        df = merge_arrhythmia_sources(base, reg)
        df["arrhythmia_source_file"] = source_name

        # Standardize IDs and genes from known CAB outputs.
        coalesce_existing(df, "assertion_id", ["assertion_id", "cab_assertion_id", "variant_assertion_id"], "")
        if df["assertion_id"].astype(str).eq("").all():
        ...
        coalesce_existing(df, "gene", ["gene", "gene_symbol", "GeneSymbol", "gene_baseline", "symbol"], "")

        # Baseline-only portability features.
        coalesce_existing(df, "baseline_portability_score", [
            "baseline_portability_score", "CPI_baseline_only", "cab_portability_index",
            "cab_portability_index_baseline_only", "mean_cab_portability_index", "CPI"
        ], np.nan)
        df["baseline_portability_score"] = pd.to_numeric(df["baseline_portability_score"], errors="coerce")
        coalesce_existing(df, "baseline_nonportability_score", [
            "baseline_nonportability_score", "nonportability_score_baseline_only", "nonportability_score"
        ], np.nan)
        df["baseline_nonportability_score"] = pd.to_numeric(df["baseline_nonportability_score"], errors="coerce")
        missing_nonport = df["baseline_nonportability_score"].isna() & df["baseline_portability_score"].notna()
        df.loc[missing_nonport, "baseline_nonportability_score"] = 100 - df.loc[missing_nonport, "baseline_portability_score"]
        coalesce_existing(df, "baseline_regime_primary", [
            "baseline_regime_primary", "CPI_tier_baseline_only", "primary_regime", "cab_portability_band",
            "failure_topology", "baseline_architecture_family"
        ], "unavailable")
        coalesce_existing(df, "baseline_architecture_family", [
            "baseline_architecture_family", "primary_regime", "causal_architecture", "failure_topology", "CPI_tier_baseline_only"
        ], "unavailable")

        # Endpoints from raw ClinVar temporal rebuild / cross-environment outputs.
        coalesce_existing(df, "future_condition_label_drift", [
            "future_condition_label_drift", "condition_label_change", "condition_label_change_by_followup",
            "condition_label_change_normalized_by_followup"
        ], False)
        df["future_condition_label_drift"] = df["future_condition_label_drift"].map(norm_bool)
        coalesce_existing(df, "future_cross_environment_drift", [
            "future_cross_environment_drift", "cross_environment_drift", "cross_environment_drift_by_followup"
        ], False)
        df["future_cross_environment_drift"] = df["future_cross_environment_drift"].map(norm_bool)
        coalesce_existing(df, "future_any_meaning_drift", [
            "future_any_meaning_drift", "any_meaning_drift", "any_assertion_meaning_drift_by_followup"
        ], np.nan)
        if df["future_any_meaning_drift"].isna().all():
            df["future_any_meaning_drift"] = df["future_condition_label_drift"] | df["future_cross_environment_drift"]
        else:
            df["future_any_meaning_drift"] = df["future_any_meaning_drift"].map(norm_bool)
        coalesce_existing(df, "self_loop_stable", ["self_loop_stable", "stable_environment"], np.nan)
        if df["self_loop_stable"].isna().all():
            df["self_loop_stable"] = ~df["future_cross_environment_drift"]
        else:
            df["self_loop_stable"] = df["self_loop_stable"].map(norm_bool)
        coalesce_existing(df, "environment_baseline", [
            "environment_baseline", "condition_environment_baseline", "baseline_environment", "primary_environment", "environment_from"
        ], "unknown")

        out = standardize_decision_source(df, domain)
        # Require real arrhythmia endpoints, not all-default placeholders.
        if len(out) == 0 or (not out["future_condition_label_drift"].any() and not out["future_cross_environment_drift"].any()):
            raise ValueError(
                "Inherited-arrhythmia source loaded but did not expose nonzero future drift endpoints. "
                "Use data/processed/cab_predictive_operational_framework.csv or a decision source with condition_label_change/cross_environment_drift columns."
            )
        return out
...
def build_decision_challenge():
    ...
    missing = []
    for d in domains:
        f = load_domain_records(d)
        if f.empty:
            missing.append(d)
        else:
            frames.append(f)
    if missing:
        raise ValueError(f"Decision challenge must be three-domain; missing domains: {missing}")
    ...
    for system in ["baseline_system", "cab"]:
        if system == "baseline_system":
            unsupported = df["baseline_unsupported_deterministic_reuse"]
            repair_pred = df["baseline_system_contextual_repair_required"]
            high_meaning = df["baseline_system_high_meaning_drift_risk"]
            high_cross = df["baseline_system_high_cross_environment_drift_risk"]
            direct_reuse = df["baseline_system_direct_reuse_allowed"]
        else:
            unsupported = df["cab_unsupported_deterministic_reuse"]
            repair_pred = df["cab_contextual_repair_required"]
            high_meaning = df["cab_high_future_meaning_drift_risk"]
            high_cross = df["cab_high_future_cross_environment_drift_risk"]
            direct_reuse = df["cab_direct_single_model_reuse_allowed"]
        summary_rows.append({
            "system": system,
            "N": total,
            ...
            "self_loop_direct_reuse_rate": float((df["routing_gold_direct_reuse_allowed"] & direct_reuse).sum() / max(1, df["routing_gold_direct_reuse_allowed"].sum())),
        })
    ...
    for domain, sub in df.groupby("domain"):
        for system in ["baseline_system", "cab"]:
            if system == "baseline_system":
                unsupported = sub["baseline_unsupported_deterministic_reuse"]
                repair_pred = sub["baseline_system_contextual_repair_required"]
                high_cross = sub["baseline_system_high_cross_environment_drift_risk"]
            else:
                unsupported = sub["cab_unsupported_deterministic_reuse"]
                repair_pred = sub["cab_contextual_repair_required"]
                high_cross = sub["cab_high_future_cross_environment_drift_risk"]
            domain_rows.append({
                "domain": domain,
                "system": system,
                "N": len(sub),
